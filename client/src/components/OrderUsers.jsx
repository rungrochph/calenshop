...
export default function OrderUsers() {
  ...
  const openModal2 = (clickInfo, id) => {
    ...
    setOrderId({ id: id, order_status: 5 });
    console.log("Id", id);
  };
  const openModal3 = (clickInfo, id, name, price, status) => {
      ...
      cell: (row) => (
        <div className="flex flex-col space-x-4 p-1">
          {row.order_status === "รอจ่ายเงิน" && (
            ...
            >
              <p className="text-green-500">จ่่ายเงิน</p>
            </button>
          )}

          {row.order_status === "กำลังจัดส่ง" && (
            <button
            ...
            >
              <p className="text-purple-500">รับแล้ว</p>
            </button>
          )}
          {
            <button
  ...
  return (
    <div className="bg-gray-50 pb-20">
      ...
      {isModalOpen && (
        <dialog className="modal " open>
          <div className="modal-box">
            ...
            <p className="py-4">โอนเงินไปยัง พพ: 0885554528 </p>
            <p className="py-4 mt-4 lg:ml-16">
              ชื่อบช: นายบัญชี เงินล้าน ธนาคารกรุงโรม{" "}
            </p>
            <p className="py-4 mt-4 lg:ml-16">จากนั่นกดปุ่มยืนยัน </p>
      ...
      {isModalOpen2 && (
        <dialog className="modal " open>
          <div className="modal-box">
            <h3 className="font-bold text-lg ">ยืนยันการได้รับสินค้า</h3>
            <div className="modal-action">
              <button className="btn btn-success mr-4" onClick={updateStatusto}>
                ยืนยันการรับสินค้า
              </button>
      ...
      {isModalOpen3 && (
        <dialog className="modal " open>
          <div className="modal-box">
            ...
            <p>สถานะ : {orderData.order_status}</p>
            {/* Additional details go here */}
            <div className="modal-action">
              <button className="btn" onClick={() => setIsModalOpen3(false)}>
                Close
              </button>
            </div>
          </div>
